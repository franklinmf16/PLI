...
compileVariableDeclarationPart slot symbols (x : xs) =
    let
        (x0, x1) = x
        (slot', symbols') = compileVariableDeclarationList slot symbols x0 x1
    in
        compileVariableDeclarationPart slot' symbols' xs

compileVariableDeclarationList ::
    Int -> Symbols -> ASTIdentifierList -> ASTTypeDenoter -> (Int, Symbols)
compileVariableDeclarationList slot symbols [] _ =
    (slot, symbols)
compileVariableDeclarationList slot (procSymbols, varSymbols) (x : xs) typeDenoter =
    let
        varSymbols' = Map.insert x (False, typeDenoter, slot) varSymbols
    in
        compileVariableDeclarationList (slot + 1) (procSymbols, varSymbols') xs typeDenoter
